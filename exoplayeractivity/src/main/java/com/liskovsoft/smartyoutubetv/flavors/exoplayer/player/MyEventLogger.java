...
/* package */ final class MyEventLogger implements ExoPlayer.EventListener, AudioRendererEventListener, VideoRendererEventListener,
    ...
    public void onTimelineChanged(Timeline timeline, Object manifest, int reason) {
        ...
        for (int i = 0; i < Math.min(windowCount, MAX_TIMELINE_ITEM_LINES); i++) {
            ...
            Log.d(TAG, "  " + "window [" + getTimeString(mWindow.getDurationMs()) + ", " + mWindow.isSeekable + ", " + mWindow.isDynamic + "]");
        }
    ...
    public void onPlayerError(ExoPlaybackException e) {
        ...
        //        Log.e(TAG, "Playback error: TYPE_SOURCE: " + e.getSourceException().getMessage());
        //        break;
        //
        //    case ExoPlaybackException.TYPE_RENDERER:
        //        Log.e(TAG, "Playback error: TYPE_RENDERER: " + e.getRendererException().getMessage());
        //        break;
        //
        //    case ExoPlaybackException.TYPE_UNEXPECTED:
        //        Log.e(TAG, "Playback error: TYPE_UNEXPECTED: " + e.getUnexpectedException().getMessage());
        //        break;
        //}
    }
            ...
            if (rendererTrackGroups.length > 0) {
                ...
                for (int groupIndex = 0; groupIndex < rendererTrackGroups.length; groupIndex++) {
                    ...
                    for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
                        ...
                        Log.d(TAG,
                                "      " + status + " Track:" + trackIndex + ", " + Format.toLogString(trackGroup.getFormat(trackIndex)) + ", " +
        ...
        if (unassociatedTrackGroups.length > 0) {
            ...
            for (int groupIndex = 0; groupIndex < unassociatedTrackGroups.length; groupIndex++) {
                ...
                for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
                    ...
                    Log.d(TAG, "      " + status + " Track:" + trackIndex + ", " + Format.toLogString(trackGroup.getFormat(trackIndex)) + ", " +
                            "supported=" + formatSupport);
                }
                Log.d(TAG, "    ]");
            }
    ...
    private void printMetadata(Metadata metadata, String prefix) {
        for (int i = 0; i < metadata.length(); i++) {
            ...
            if (entry instanceof TextInformationFrame) {
                ...
                Log.d(TAG, prefix + String.format("%s: mimeType=%s, filename=%s, description=%s", geobFrame.id, geobFrame.mimeType,
    ...
    private static String getTrackStatusString(TrackSelection selection, TrackGroup group, int trackIndex) {
        return getTrackStatusString(selection != null && selection.getTrackGroup() == group && selection.indexOf(trackIndex) != C.INDEX_UNSET);
    }
    ...
    @Override
    public void onLoadError(int windowIndex, @Nullable MediaPeriodId mediaPeriodId, LoadEventInfo loadEventInfo, MediaLoadData mediaLoadData,
